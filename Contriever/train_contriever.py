...
from src import data, beir_utils, slurm, dist_utils, utils, finetuning_data, data
...
def eval_model(opt, query_encoder, doc_encoder, tokenizer, tb_logger, step):
    for datasetname in opt.eval_datasets:
        ...
        if dist_utils.is_main():
            ...
            if not os.path.exists(f"{opt.output_dir}checkpoint/"):
                os.mkdir(f"{opt.output_dir}checkpoint/")
                
            if not os.path.exists(f"{opt.output_dir}checkpoint/step-{step}/"):
                os.mkdir(f"{opt.output_dir}checkpoint/step-{step}/")

            save_path = f"{opt.output_dir}checkpoint/step-{step}/results.json"
...
if __name__ == "__main__":
    ...
    if opt.model_path == "none":
        ...
        step = 0
    elif opt.continue_training:
        model_path = os.path.join(opt.output_dir, "checkpoint", "lastlog")
