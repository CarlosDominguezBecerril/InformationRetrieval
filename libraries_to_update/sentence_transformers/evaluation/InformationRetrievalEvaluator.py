...
class InformationRetrievalEvaluator(SentenceEvaluator):
    ...
    def __call__(self, model, output_path: str = None, epoch: int = -1, steps: int = -1, *args, **kwargs) -> float:
        ...
        if self.main_score_function is None:
            return max([scores[name][self.main_metric][max(self.metric_to_at_k[self.main_metric])] for name in self.score_function_names])
        else:
            return scores[self.main_score_function][self.main_metric][max(self.metric_to_at_k[self.main_metric])]
