...
class TrainRetriever:
    ...
    def __init__(self, model: Type[SentenceTransformer], batch_size: int = 64, sep: str = " ", is_dpr: bool = False):
        self.model = model
            ...
            for query_id in query_ids_batch:
                for corpus_id, score in qrels[query_id].items():
                    if score >= 1: # if score = 0, we don't consider for training
                        try:
                            s1 = queries[query_id]
                            s2 = corpus[corpus_id].get("title") + self.sep + corpus[corpus_id].get("text") 
                            if self.is_dpr:
                                train_samples.append(InputExample(guid=idx, texts=[{"query": s1}, {"doc": s2}], label=1))
                            else:
    ...
    def load_ir_evaluator(self, corpus: Dict[str, Dict[str, str]], queries: Dict[str, str], 
                 qrels: Dict[str, Dict[str, int]], max_corpus_size: int = None, name: str = "eval", dev_batch_size: int = 32, main_score_function: str = "dot_score", main_metric: str = "ndcg@k") -> SentenceEvaluator:
        ...
        if self.is_dpr:
            corpus = {idx: {"doc":corpus[idx].get("title") + self.sep + corpus[idx].get("text")} for idx in corpus}
        else:   
            corpus = {idx: corpus[idx].get("title") + self.sep + corpus[idx].get("text") for idx in corpus}
        ...
        if self.is_dpr:
            for qid in queries.keys():
                queries[qid] = {"query": queries[qid]}

        logger.info("{} set contains {} documents and {} queries".format(name, len(corpus), len(queries)))
        return InformationRetrievalEvaluator(queries, corpus, rel_docs, name=name, batch_size=dev_batch_size, main_score_function=main_score_function, main_metric=main_metric)
