...
class SentenceBERT:
    def __init__(self, model_path: Union[str, Tuple] = None, sep: str = " ", is_dpr: bool = False, **kwargs):
        self.sep = sep
        self.is_dpr = is_dpr
    ...
    def encode_queries(self, queries: List[str], batch_size: int = 16, **kwargs) -> Union[List[Tensor], np.ndarray, Tensor]:
        if self.is_dpr:
            return self.q_model.encode([{"query": query} for query in queries], batch_size=batch_size, **kwargs).cpu()
        else:
    ...
    def encode_corpus(self, corpus: Union[List[Dict[str, str]], Dict[str, List]], batch_size: int = 8, **kwargs) -> Union[List[Tensor], np.ndarray, Tensor]:
        ...
        if self.is_dpr:
            return self.doc_model.encode([{"doc": doc} for doc in sentences], batch_size=batch_size, **kwargs).cpu()
        else:
    ...
    def encode_corpus_parallel(self, corpus: Union[List[Dict[str, str]], Dataset], pool: Dict[str, str], batch_size: int = 8, chunk_id: int = None, **kwargs):
        ...
        if self.is_dpr:
            sentences = [{"doc": doc} for doc in sentences]
